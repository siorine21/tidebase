...
```
GET /api/v1/spots/{spot_id}/tide?date=2026-07-17   // 要認証（スポット参照のため）
```

- サーバー側で spot → `tide_station_code` を解決（未設定ならリクエスト時に最近傍を再計算）。
  淡水スポットは 400（`"淡水スポットは潮汐データの対象外です"`）
- 既存の `GET /api/v1/tide?station=` 直接指定も維持（地点切替タブ用・認証不要）
- ※ v1.0 初版では `?spot_id=` クエリ案だったが、認証の要否がパラメータで変わる
  設計を避けるため実装時にパス形式へ変更
